...
void load_image_points(int board_width, int board_height, int num_imgs, float square_size,
    ...
    TermCriteria termCrit(TermCriteria::EPS | TermCriteria::COUNT, 30, 0.1);
    ...
    for (int i = 1; i <= num_imgs; i++) {
        ...
        found1 = findChessboardCorners(img1, board_size, corners1, chessBoardFlags);
        found2 = findChessboardCorners(img2, board_size, corners2, chessBoardFlags);
        ...
        cornerSubPix(gray1, corners1, cv::Size(5, 5), cv::Size(-1, -1), termCrit);
        drawChessboardCorners(gray1, board_size, corners1, found1);
        cornerSubPix(gray2, corners2, cv::Size(5, 5), cv::Size(-1, -1), termCrit);
